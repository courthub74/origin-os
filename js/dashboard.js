...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderAttention(items = []) {
    ...
    for (const it of items.slice(0, 5)) {
      ...
      row
        ...
        ?.addEventListener("click", () => navTo(it.cta?.go || "create.html", it.workId));
  ...
  try {
    ...
    console.warn("renderNextAction failed:", e);
  }

  try {
    renderAttention(dash.attention);
  } catch (e) {
    console.warn("renderAttention failed:", e);
  }

  try {
    renderContinue(dash.continue);
  } catch (e) {
    console.warn("renderContinue failed:", e);
  }
  ...
  async function loadRecentImageSrc(item) {
    if (item.imageFileId) {
      const token = getAccessToken();
      if (!token) throw new Error("Missing token for image fetch");
  ...
  function initActivityModal() {
    ...
    activityModalEdit?.addEventListener("click", () => {
      ...
      // Keep this aligned with the rest of your dashboard navigation pattern.
